...
| Jev call shape | `experimental_evaluate` from `ai`; takes `model`, `state`, `questions`, `abortSignal`, `maxRetries`; a `choice` question returns `answers.<id>.choice` + `probabilities`; TypeSafe confidence at `result.providerMetadata?.typesafe?.confidence` | https://ai-sdk.dev/docs/ai-sdk-core/evaluation |
